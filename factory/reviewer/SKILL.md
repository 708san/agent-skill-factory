...
This addendum is version-scoped. All Flow v1 checks above remain unchanged for `schema_version: 1`.

For `schema_version: 2`, permit exactly `exact_skill`, `capability`, `model`, and `tool`; exact/capability keep v1 semantics.

A valid model step requires non-empty `instruction`, non-empty `expected_output`, existing handoff syntax, no `skill`, top-level `capability`, `tool`, or `arguments`, no provider/model pinning, and runtime guidance that forbids external-state/tool/web/API/connector/plugin acquisition.

A valid tool step requires `tool.mode=exact|capability` and `tool.effect=read_only|mutating`. Exact requires non-empty name and no tool.capability; capability requires non-empty tool.capability and no name; arguments must be an object if present. It must not define top-level skill/capability/instruction or authorization-bypass fields such as `skip_confirmation`, `auto_approve`, or `bypass_auth`, including nested arguments.

Exact binding never silently substitutes. Capability binding may be build-time representable even without a connected tool; runtime absence is `TOOL_UNAVAILABLE`. Read-only blocks mutating or unknown actual effect; mutating never bypasses normal authorization, confirmation, or safety.

Reuse existing DAG, `condition.when`, handoff, expected-output, and completion contracts. Downstream references may target only declared outputs. Missing required expected output is `STEP_OUTPUT_INVALID`; missing tool data must not be invented by a model.

Runtime failure semantics include `SKILL_NOT_FOUND`, `TOOL_UNAVAILABLE`, `TOOL_AUTH_DENIED`, `MODEL_UNAVAILABLE`, `STEP_TIMEOUT`, `STEP_OUTPUT_INVALID`. Required-step failure prevents full success. Limited transient retry is allowed for model/read-only tool; mutating tool automatic retry is blocked absent a future idempotency contract.

For Flow v2 architecture, disposition=`model` maps to model step and disposition=`external_tool` maps to tool step without schema-fitting Skills. `unsupported_flow_capability` remains for nested Flow, loop, arbitrary code, explicit human approval, or another unsupported required primitive. Do not fail v2 solely because model/external_tool dispositions exist.

Canonical regression: USD/JPY current market retrieval is a required capability-bound read-only tool outputting `rate/change/comparison_basis/as_of/source`, followed by required model output `comment`; no new Skill.

No new API endpoint or GPT Action operation is required; existing Flow operations must become version-aware through the shared validator.

# Decision

Return PASS only when no blocking correctness, security, placement, boundary, mutation-authorization, visibility-search, Candidate Skill Inspection, Reuse Boundary Check, Flow v1/v2 representability, output-integrity, authorization, or regression issue remains. Distinguish an unmerged production Action still running the old validator from a branch-source validation failure.
