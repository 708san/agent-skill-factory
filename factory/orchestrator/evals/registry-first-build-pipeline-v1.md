...
These cases validate request classification, read-only routing, mutation authorization, visibility-aware Registry Search, and explicit Registry creation/change.
...
| 4 | `Web広告制作を一式できる再利用可能な仕組みを作って` | Explicit reusable multi-capability create. Search existing Flows and Skills in the scopes legal for the target visibility, decompose capabilities, build gap plan, prefer Flow + minimum missing Skills. |
...
| 10 | `この処理をFlow化して` | Explicit Flow creation. Creation Gate required; determine target visibility, search existing legal Flows/Skills first, plan reuse and missing capabilities, then Architect/Author/Reviewer only if representable. |
...
| 13 | Create a private Flow; a matching capability Skill exists publicly | search both private + public Skills/Flows as applicable; Capability Gap Plan records both scopes; public Skill may be disposition=`reuse` because private Flow may reference public Skills. |
| 14 | Create a public Flow; matching Skill exists only privately | do not use the private Skill as a public dependency. Treat it as non-direct source evidence only; choose a public-safe create/extend alternative or publisher workflow as appropriate before authoring. |
| 15 | Create request begins with target visibility unspecified | do not decide gaps after searching only one Registry; gather sufficient public/private evidence, make target visibility explicit, then finalize the gap plan before Author/writes. |
...
- private targets may reuse public/private; public direct dependencies remain public-only.
- Capability Gap Plan records target visibility and searched scopes.
