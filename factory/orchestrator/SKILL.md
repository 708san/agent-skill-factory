...
This addendum is version-scoped. It does not modify Flow v1 semantics above. Any v1-only statement about direct `model`/`external_tool` representability applies only to `schema_version: 1`.

`schema_version: 2` supports `exact_skill`, `capability`, `model`, and `tool`. `exact_skill` and `capability` retain v1 semantics.

For Flow v2, Capability Gap Plan disposition=`model` maps directly to a `model` step and disposition=`external_tool` maps directly to a `tool` step. Do not create schema-fitting Skills merely to represent either disposition.

A v2 `model` step is pure LLM-native execution: non-empty `instruction`, normal `input_handoff`, non-empty `expected_output`, no pinned provider/model, and no web/API/connector/plugin/current-state acquisition.

A v2 `tool` step uses `tool.mode=capability|exact` and `tool.effect=read_only|mutating`. Capability binding is portable runtime resolution; exact binding must fail rather than silently substitute. `effect` is a maximum permitted effect, not authorization. Read-only blocks mutating or unknown-effect tools. Mutating does not bypass platform/tool auth, user confirmation, or safety policy.

Build-time representability is distinct from runtime availability. A capability-bound tool step may validate even when no matching tool is currently connected; runtime absence is `TOOL_UNAVAILABLE`.

Reuse existing DAG, condition, handoff, expected-output, and completion semantics. Downstream steps may use only declared outputs. Missing required output is `STEP_OUTPUT_INVALID`; do not infer missing tool data with a model.

Flow runtime failure semantics include `SKILL_NOT_FOUND`, `TOOL_UNAVAILABLE`, `TOOL_AUTH_DENIED`, `MODEL_UNAVAILABLE`, `STEP_TIMEOUT`, and `STEP_OUTPUT_INVALID`. Required-step failure prevents full success. Limited transient retry is allowed for model/read-only tool; mutating tool automatic retry remains disabled absent a future idempotency contract.

`unsupported_flow_capability` remains for required primitives outside the selected schema/runtime, including nested Flow execution, loops, arbitrary code, explicit human-approval steps, or another unsupported primitive. In Flow v2, `model`/`external_tool` dispositions alone are not blockers.
