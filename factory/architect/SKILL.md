...
This addendum is version-scoped. All Flow v1 statements above remain authoritative for `schema_version: 1`. The v1-only representability checks do not block v2 merely because a required capability is `model` or `external_tool`.

For `schema_version: 2`, supported step types are `exact_skill`, `capability`, `model`, and `tool`. Existing exact/capability semantics are unchanged.

Map Capability Gap Plan dispositions directly:
...
A model step must be pure LLM-native work with non-empty instruction and outputs, no provider/model pinning, and no direct external-state/tool access. A tool step uses capability or exact binding plus `read_only|mutating` maximum effect. Capability binding may be build-time representable without a currently connected tool; exact binding availability is likewise a runtime concern when no authoritative tool catalog exists. Runtime absence is `TOOL_UNAVAILABLE`, not an architecture blocker.

Tool effect never grants permission. `read_only` fails closed for mutating or unknown actual effect. `mutating` still obeys normal authorization, confirmation, and safety policy; no bypass mechanism is permitted.

Reuse the existing DAG, `condition.when`, handoff, expected-output, and completion contracts. Missing declared runtime output is `STEP_OUTPUT_INVALID`; do not synthesize missing tool data with a model.

Flow runtime failures include `SKILL_NOT_FOUND`, `TOOL_UNAVAILABLE`, `TOOL_AUTH_DENIED`, `MODEL_UNAVAILABLE`, `STEP_TIMEOUT`, and `STEP_OUTPUT_INVALID`. Required-step failure prevents full success. Limited transient retry is allowed for model/read-only tool steps; mutating tool automatic retry is not allowed absent a future idempotency contract.

`unsupported_flow_capability` remains for required nested Flow execution, loops, arbitrary code, explicit human-approval steps, or another execution primitive unsupported by the selected schema/runtime.

Canonical USD/JPY architecture: current rate/change retrieval remains disposition=`external_tool` and becomes a required capability-bound read-only tool step; short Japanese commentary remains disposition=`model` and becomes a required model step. No new Skill is justified solely for either step.
