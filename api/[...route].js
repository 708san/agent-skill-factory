...
async function handleCreateBranch(request) { requirePost(request); const body=await readJson(request); const repo=body.target==='factory'?config().factoryRepo:repoForVisibility(body.visibility); const branch=body.branch; const result=await createBranch(repo,branch,body.base||config().baseBranch); return json({ok:true,repository:`${config().owner}/${repo}`,branch,status:result.status,baseSha:result.baseSha||null,mergeBaseSha:result.mergeBaseSha||null,aheadBy:result.aheadBy??null,behindBy:result.behindBy??null,stale:result.stale??null}); }
