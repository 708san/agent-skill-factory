...
export function assertSkillRegistryPath(path, content) {
  if (typeof path !== 'string' || !path) throw new Error('Skill path is required');
  if (path.startsWith('/') || path.includes('\\')) throw new Error(`Invalid Skill registry path: ${path}`);

  const segments = path.split('/');
  if (segments.includes('..') || segments.includes('.') || segments.some((segment) => !segment)) {
    throw new Error(`Invalid Skill registry path: ${path}`);
  }

  if (segments.length < 3 || segments[0] !== 'skills') {
    throw new Error(`Skill package files must be under skills/<skill-name>/: ${path}`);
  }
  ...
  if (segments.includes('SKILL.md') && !(segments.length === 3 && segments[2] === 'SKILL.md')) {
    throw new Error(`SKILL.md must be at skills/${skillName}/SKILL.md`);
  }

  if (segments.length === 3 && segments[2] === 'SKILL.md' && typeof content === 'string') {
    const frontmatterName = frontmatterScalar(content, 'name');
    if (frontmatterName && frontmatterName !== skillName) {
      throw new Error(`Skill frontmatter name must match directory name: ${skillName}`);
    }
  }
