...
const FLOW_STEP_TYPES = {
  1: new Set(['exact_skill', 'capability']),
  2: new Set(['exact_skill', 'capability', 'model', 'tool'])
};
...
export const FLOW_RUNTIME_FAILURE_CODES = Object.freeze({
...
export async function findRegistryDependents({ targetType, targetName, targetVisibility, dependentVisibility, ref }) {
  ...
  const target = { type: targetType, name: targetName, visibility: targetVisibility };
  const dependents = [];
  if (targetType === 'skill') {
    for (const flowName of await listFlows(dependentVisibility, ref)) {
      ...
      steps.forEach((step, stepIndex) => {
        ...
        pushReferenceIfMatch(dependents, { target, ownerVisibility: dependentVisibility, reference: step.skill, dependentType: 'flow', dependentName: flowName, manifestPath: `flows/${flowName}/FLOW.json`, referenceKind: 'exact_skill', referenceLocation: { step_id: typeof step.id === 'string' ? step.id : null, step_index: stepIndex } });
      });
    }
  }
  for (const suiteName of await listSuites(dependentVisibility, ref)) {
    ...
    const members = Array.isArray(suite.manifest?.members?.[memberKey]) ? suite.manifest.members[memberKey] : [];
    members.forEach((member, memberIndex) => {
      if (!isObject(member)) return;
      pushReferenceIfMatch(dependents, { target, ownerVisibility: dependentVisibility, reference: member, dependentType: 'suite', dependentName: suiteName, manifestPath: `suites/${suiteName}/SUITE.json`, referenceKind: targetType === 'skill' ? 'suite_skill_member' : 'suite_flow_member', referenceLocation: { member_index: memberIndex } });
    });
  }
  return dependents;
}
...
export async function preflightRegistryDelete({ path, visibility, branch, dependencyRefs = {} }) {
  const target = primaryDeleteTarget(path, visibility);
  if (!target) return { guarded: false, safe: true, blocked: false, reason: 'not_primary_registry_object' };
  assertVisibility(visibility, 'visibility');
  if (typeof branch !== 'string' || !branch.trim()) return blockedDelete('missing_delete_ref', target, [], 'Delete branch/ref is required');
  ...
  for (const spec of scanSpecs) {
    const repo = repoForVisibility(spec.visibility);
    try { await assertRefExists(repo, spec.ref); }
    catch (e) { scans.push({ visibility: spec.visibility, ref: spec.ref, status: 'ref_check_failed', dependent_count: null, dependents: [], error: e.message }); return blockedDelete('dependency_scan_incomplete', target, scans, `Dependency ref check failed for ${spec.visibility}@${spec.ref}`); }
    try {
      const dependents = await findRegistryDependents({ targetType: target.type, targetName: target.name, targetVisibility: target.visibility, dependentVisibility: spec.visibility, ref: spec.ref });
      scans.push({ visibility: spec.visibility, ref: spec.ref, status: 'complete', dependent_count: dependents.length, dependents });
...
export function validateStepOutput(step, output) {
  if (!isObject(output)) return { ok: false, errorCode: FLOW_RUNTIME_FAILURE_CODES.STEP_OUTPUT_INVALID, missingOutputs: [...(step?.expected_output || [])], output: {} };
  const expected = Array.isArray(step?.expected_output) ? step.expected_output : [];
  const missingOutputs = expected.filter((field) => !Object.prototype.hasOwnProperty.call(output, field));
  const normalized = Object.fromEntries(expected.filter((field) => Object.prototype.hasOwnProperty.call(output, field)).map((field) => [field, output[field]]));
  return { ok: missingOutputs.length === 0, errorCode: missingOutputs.length ? FLOW_RUNTIME_FAILURE_CODES.STEP_OUTPUT_INVALID : null, missingOutputs, output: normalized };
}

export function assessToolEffect({ declaredEffect, actualEffect }) {
  if (!TOOL_EFFECTS.has(declaredEffect)) return { ok: false, errorCode: FLOW_RUNTIME_FAILURE_CODES.TOOL_AUTH_DENIED, reason: 'invalid_declared_effect' };
  if (!TOOL_EFFECTS.has(actualEffect)) return { ok: declaredEffect !== 'read_only', errorCode: declaredEffect === 'read_only' ? FLOW_RUNTIME_FAILURE_CODES.TOOL_AUTH_DENIED : null, reason: 'actual_effect_unknown' };
  if (declaredEffect === 'read_only' && actualEffect !== 'read_only') return { ok: false, errorCode: FLOW_RUNTIME_FAILURE_CODES.TOOL_AUTH_DENIED, reason: 'effect_exceeds_manifest_limit' };
  return { ok: true, errorCode: null, reason: 'effect_allowed' };
}

export function flowRetryPolicy(step) {
  if (step?.type === 'model') return { automaticRetry: 'limited_transient_only' };
  if (step?.type === 'tool' && step.tool?.effect === 'read_only') return { automaticRetry: 'limited_transient_only' };
  if (step?.type === 'tool' && step.tool?.effect === 'mutating') return { automaticRetry: 'disabled_without_future_idempotency_contract' };
  return { automaticRetry: 'runtime_default' };
}
...
function primaryDeleteTarget(path, visibility) {
  if (typeof path !== 'string') return null;
  let match = path.match(/^skills\/([a-z0-9][a-z0-9-]{1,62})\/SKILL\.md$/);
  if (match) return { type: 'skill', name: match[1], visibility, path };
  match = path.match(/^flows\/([a-z0-9][a-z0-9-]{1,62})\/FLOW\.json$/);
  if (match) return { type: 'flow', name: match[1], visibility, path };
  return null;
}

function blockedDelete(reason, target, scans, message) {
  return { guarded: true, safe: false, blocked: true, reason, message, target, scans, dependent_count: scans.reduce((sum, scan) => sum + (Number.isInteger(scan.dependent_count) ? scan.dependent_count : 0), 0), dependents: scans.flatMap((scan) => Array.isArray(scan.dependents) ? scan.dependents : []) };
}

function pushReferenceIfMatch(dependents, { target, ownerVisibility, reference, dependentType, dependentName, manifestPath, referenceKind, referenceLocation }) {
...
function validateFlowShape(flow, name, errors, warnings) {
  ...
  const ids = new Set();
  const allowedTypes = FLOW_STEP_TYPES[flow.schema_version] || new Set();
...
function validateToolStep(step, errors) {
  ...
  if (step.tool.mode === 'exact') {
    if (typeof step.tool.name !== 'string' || !step.tool.name.trim()) errors.push(`Tool step ${step.id} exact mode requires non-empty tool.name`);
    if (step.tool.capability !== undefined) errors.push(`Tool step ${step.id} exact mode must not define tool.capability`);
  }
  if (step.tool.mode === 'capability') {
    if (typeof step.tool.capability !== 'string' || !step.tool.capability.trim()) errors.push(`Tool step ${step.id} capability mode requires non-empty tool.capability`);
    if (step.tool.name !== undefined) errors.push(`Tool step ${step.id} capability mode must not define tool.name`);
  }
  ...
  for (const forbidden of FORBIDDEN_TOOL_AUTH_FIELDS) {
    if (Object.prototype.hasOwnProperty.call(step, forbidden) || Object.prototype.hasOwnProperty.call(step.tool, forbidden)) errors.push(`Tool step ${step.id} must not define ${forbidden}`);
  }
}
...
function validateCompletion(completion, steps, errors) {
  ...
  if (completion.required_steps !== 'all_required') errors.push('completion.required_steps must be "all_required"');
