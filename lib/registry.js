    ...
    for (const flowName of await listFlows(dependentVisibility, ref)) {
      ...
      steps.forEach((step, stepIndex) => {
        ...
        pushReferenceIfMatch(dependents, {
          ...
          referenceLocation: { step_id: typeof step.id === 'string' ? step.id : null, step_index: stepIndex }
        });
      });
    }
  }
...
export async function preflightRegistryDelete({ path, visibility, branch, dependencyRefs = {} }) {
  ...
  if (typeof branch !== 'string' || !branch.trim()) {
    return blockedDelete('missing_delete_ref', target, [], 'Delete branch/ref is required');
  }

  const scanSpecs = visibility === 'public'
    ? [
        { visibility: 'public', ref: branch },
        { visibility: 'private', ref: dependencyRefs?.privateRef }
      ]
    : [{ visibility: 'private', ref: branch }];

  if (visibility === 'public' && (typeof dependencyRefs?.privateRef !== 'string' || !dependencyRefs.privateRef.trim())) {
    return blockedDelete('missing_dependency_ref', target, [
      { visibility: 'public', ref: branch, status: 'pending' },
      { visibility: 'private', ref: dependencyRefs?.privateRef || null, status: 'missing_ref' }
    ], 'dependencyRefs.privateRef is required for public Skill/Flow deletion');
  }
  ...
  for (const spec of scanSpecs) {
    ...
    try {
      await assertRefExists(repo, spec.ref);
    } catch (e) {
      scans.push({ visibility: spec.visibility, ref: spec.ref, status: 'ref_check_failed', dependent_count: null, dependents: [], error: e.message });
      return blockedDelete('dependency_scan_incomplete', target, scans, `Dependency ref check failed for ${spec.visibility}@${spec.ref}`);
    }
    ...
    try {
      const dependents = await findRegistryDependents({
        ...
        dependentVisibility: spec.visibility,
        ref: spec.ref
      });
      scans.push({ visibility: spec.visibility, ref: spec.ref, status: 'complete', dependent_count: dependents.length, dependents });
    } catch (e) {
      scans.push({ visibility: spec.visibility, ref: spec.ref, status: 'scan_failed', dependent_count: null, dependents: [], error: e.message });
      return blockedDelete('dependency_scan_incomplete', target, scans, `Dependency scan failed for ${spec.visibility}@${spec.ref}`);
    }
  }
  ...
  return {
    ...
    safe: true,
    blocked: false,
    reason: 'dependency_preflight_complete',
    target,
    scans,
    dependent_count: 0,
    dependents: []
...
export function validateStepOutput(step, output) {
  ...
  if (!isObject(output)) return { ok: false, errorCode: FLOW_RUNTIME_FAILURE_CODES.STEP_OUTPUT_INVALID, missingOutputs: [...expected], output: {} };
...
function blockedDelete(reason, target, scans, message) {
  return {
    ...
    target,
    scans,
    dependent_count: scans.reduce((sum, scan) => sum + (Number.isInteger(scan.dependent_count) ? scan.dependent_count : 0), 0),
    dependents: scans.flatMap((scan) => Array.isArray(scan.dependents) ? scan.dependents : [])
  };
}
...
function validateToolStep(step, errors) {
  ...
  const forbidden = findForbiddenAuthField(step);
  if (forbidden) errors.push(`Tool step ${step.id} must not define ${forbidden}`);
}
...
function findForbiddenAuthField(value) {
  if (Array.isArray(value)) {
    for (const item of value) {
      const found = findForbiddenAuthField(item);
  ...
  for (const [key, nested] of Object.entries(value)) {
    if (FORBIDDEN_TOOL_AUTH_FIELDS.has(key)) return key;
    const found = findForbiddenAuthField(nested);
    if (found) return found;
  }
  return null;
}
