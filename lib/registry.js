...
export function assertRegistryPath(path, content) {
  ...
  assertName(name);
  if (root === 'skills') return assertSkillPath(segments, name, content);
...
function assertSkillPath(segments, name, content) {
  if (segments.includes('SKILL.md') && !(segments.length === 3 && segments[2] === 'SKILL.md')) throw new Error(`SKILL.md must be at skills/${name}/SKILL.md`);
  if (segments.length === 3 && segments[2] === 'SKILL.md' && typeof content === 'string') {
    const fm = content.match(/^---\s*\n([\s\S]*?)\n---(?:\n|$)/);
    const line = fm?.[1].split('\n').find((x) => x.trim().startsWith('name:'));
    const declared = line ? line.split(':').slice(1).join(':').trim().replace(/^['"]|['"]$/g, '') : '';
