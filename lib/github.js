...
async function gh(path, options = {}, meta = {}) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (!shouldRetryCode(error.code) || attempt === MAX_ATTEMPTS) throw error;
      lastError = error;
      logEvent('github_retry', { attempt, method, githubStatus: res.status, errorCode: error.code, operation: meta.operation || method, repo: meta.repo || null, ref: meta.ref || null, path: meta.path || null });
      ...
      if (error instanceof FactoryError) {
        ...
        const wrapped = new FactoryError(timeoutLike ? 'UPSTREAM_TIMEOUT' : 'INTERNAL_ERROR', timeoutLike ? 'GitHub request timed out' : 'GitHub network request failed', {
...
function githubError(res, data, meta) {
  ...
  if (meta.operation === 'assert_ref' && (res.status === 404 || res.status === 422)) {
    return new FactoryError('REF_NOT_FOUND', `Ref not found: ${meta.ref}`, { ...base, status: 404 });
  }
  if (meta.operation === 'create_branch' && (res.status === 409 || res.status === 422) && /(?:reference|ref).*already exists|already exists/i.test(message)) {
    return new FactoryError('REF_ALREADY_EXISTS', `Ref already exists: ${meta.ref}`, { ...base, status: 409 });
  }
  if (meta.operation === 'put_file' && (res.status === 409 || res.status === 422)) {
    return new FactoryError('FILE_CONFLICT', message, { ...base, status: 409 });
  }
...
export async function getRepo(repo) {
  ...
  catch (e) { if (e.code === 'FILE_NOT_FOUND') throw new FactoryError('REPOSITORY_NOT_FOUND', 'Repository not found or not visible to the configured credential', { ...e.details, status: 404, repo, operation: 'get_repo' }); throw e; }
}
...
export async function getContent(repo, path, ref) {
  ...
  try { return (await gh(`/repos/${config().owner}/${repo}/contents/${encodePath(path)}${q}`, {}, { repo, ref, path, operation: 'get_content' })).data; }
    ...
    if (ref) await assertRefExists(repo, ref);
    throw new FactoryError('FILE_NOT_FOUND', `GitHub 404: File not found: ${path}`, { ...e.details, status: 404, repo, ref, path, operation: 'get_content' });
  }
}
export async function getTextFile(repo, path, ref) {
  ...
  if (!data.content) throw new FactoryError('FILE_NOT_FOUND', `GitHub 404: No file content returned for ${path}`, { status: 404, repo, ref, path, operation: 'get_text_file' });
...
export async function assertRefExists(repo, ref) {
  ...
  try { await gh(`/repos/${config().owner}/${repo}/commits/${encodeURIComponent(ref)}`, {}, { repo, ref, operation: 'assert_ref' }); return true; }
    ...
    if (e.code === 'REF_NOT_FOUND') throw e;
    if (e.code === 'FILE_NOT_FOUND') throw new FactoryError('REF_NOT_FOUND', `Ref not found: ${ref}`, { ...e.details, status: 404, repo, ref, operation: 'assert_ref' });
    throw e;
  }
}
export async function createBranch(repo, branch, baseBranch = config().baseBranch) {
  ...
  try {
    ...
    return { status: 'created', ref: created.data.ref, object: created.data.object, baseSha: base.object.sha, mergeBaseSha: base.object.sha, aheadBy: 0, behindBy: 0, stale: false };
  } catch (e) {
    if (e.code !== 'REF_ALREADY_EXISTS') throw e;
    let existing;
    let relation;
    try {
      existing = await getBranch(repo, branch);
      relation = await compare(repo, baseBranch, branch);
    } catch (relationError) {
      throw new FactoryError('REF_ALREADY_EXISTS', `Ref already exists and its relationship to requested base ${baseBranch} could not be safely confirmed`, {
        status: 409, repo, ref: branch, operation: 'create_branch', stage: 'idempotency_check', cause: relationError
      });
    }
    const mergeBaseSha = relation.merge_base_commit?.sha || null;
    const aheadBy = relation.ahead_by ?? null;
    const behindBy = relation.behind_by ?? null;
    const stale = behindBy !== 0;
    const safeToReuse = mergeBaseSha === base.object.sha && behindBy === 0;
    if (!safeToReuse) {
      throw new FactoryError('REF_ALREADY_EXISTS', `Ref already exists but is not a non-stale descendant of requested base ${baseBranch}`, {
        status: 409, repo, ref: branch, operation: 'create_branch', stage: 'idempotency_check'
      });
    }
    return { status: 'already_exists', ref: existing.ref, object: existing.object, baseSha: base.object.sha, mergeBaseSha, aheadBy, behindBy, stale };
  }
}
...
function shouldRetryCode(code) { return ['RATE_LIMITED','UPSTREAM_TIMEOUT','UPSTREAM_5XX'].includes(code); }
