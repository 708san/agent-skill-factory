...
import { getTextFile, putTextFileIfSha } from './github.js';
...
export async function planRegistryReferenceMigration(request) {
  ...
  for (const [manifestPath, refs] of grouped) {
    try {
      const preview = await buildManifestPreview(input, manifestPath, refs, destination);
      files.push(preview);
...
export async function applyRegistryReferenceMigration(request) {
  ...
  try {
    ...
    const verification = await verifyMigration(input, plan.files.map((file) => file.manifest_path));
    return {
      ok: false,
      blocked: true,
  ...
  const verification = await verifyMigration(input, plan.files.map((file) => file.manifest_path));
  return {
    ok: verification.validation_results.every((item) => item.ok),
    blocked: false,
    migrated_count: writes.length,
    writes,
    ...verification
...
async function buildManifestPreview(input, manifestPath, refs, destination) {
  ...
  const baseline = dependentType === 'flow'
    ? await validateFlowPackage({
        visibility: input.dependentVisibility,
        name: refs[0].dependent_name,
        ref: input.dependentRef,
        flowJson: loaded.content
      })
    : await validateSuitePackage({
        visibility: input.dependentVisibility,
        name: refs[0].dependent_name,
        ref: input.dependentRef,
        suiteJson: loaded.content
      });
  ...
  const validationPreview = await validateProposedManifest({
    input,
    dependentType,
...
async function validateProposedManifest({ input, dependentType, dependentName, proposedContent, baseline, destination }) {
  if (input.to.ref === input.dependentRef) {
    const direct = dependentType === 'flow'
      ? await validateFlowPackage({
          visibility: input.dependentVisibility,
          name: dependentName,
          ref: input.dependentRef,
          flowJson: proposedContent
        })
      : await validateSuitePackage({
          visibility: input.dependentVisibility,
          name: dependentName,
          ref: input.dependentRef,
          suiteJson: proposedContent
        });
    return { ok: direct.ok, mode: 'direct', result: direct };
  }

  return {
    ok: baseline.ok && destination.validation?.ok === true,
    mode: 'composite_cross_ref',
    baseline_validation: baseline,
    destination_validation: destination.validation,
    note: 'Only structured Registry references are changed; destination references are validated at to.ref and unchanged references retain dependentRef.'
  };
}
...
async function verifyMigration(input, manifestPaths) {
  const validationResults = [];
  for (const path of manifestPaths) {
    const parsed = parseManifestPath(path);
    try {
      if (parsed.type === 'flow') {
        const current = await getFlow(input.dependentVisibility, parsed.name, input.dependentRef);
        const result = await validateProposedManifest({
          input,
          dependentType: 'flow',
          dependentName: parsed.name,
          proposedContent: current.content,
          baseline: { ok: true, mode: 'post_write_structural' },
          destination: await inspectEndpoint(input.targetType, input.to, true)
        });
        validationResults.push({ manifest_path: path, ok: result.ok, validation: result });
      } else {
        const current = await getSuite(input.dependentVisibility, parsed.name, input.dependentRef);
        const result = await validateProposedManifest({
          input,
          dependentType: 'suite',
          dependentName: parsed.name,
      ...
      validationResults.push({ manifest_path: path, ok: false, error: e.message });
    }
  }
